...
export function DesignSummary({ projectId }: DesignSummaryProps) {
  ...
  const handleExportPDF = () => {
    const downloadRecord = {
      ...
      projectName: summaryData.overview?.projectName || "Untitled Project", // Access projectName from overview
      downloadedAt: new Date().toISOString(),
      downloadId: `${projectId}-${Date.now()}`, // Use projectId from props
    }
    ...
    const updatedDownloads = [downloadRecord, ...existingDownloads].slice(0, 20) // Keep only last 20 downloads

    // Save to localStorage
    localStorage.setItem("downloadedSummaries", JSON.stringify(updatedDownloads))

    // Show download confirmation
    alert("Export functionality - connect to PDF generation service")
  }
              ...
              <>
                ...
                {hasContent(summaryData.overview.projectName) && (
                  <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-6 border border-emerald-300">
                    <h2 className="text-2xl font-bold text-emerald-900 mb-3">{summaryData.overview.projectName}</h2>
                    {hasContent(summaryData.overview.description) && (
                      <p className="text-emerald-800 leading-relaxed text-base">{summaryData.overview.description}</p>
                    )}
                  </div>
                )}
                  ...
                  {hasContent(summaryData.overview.goal) && (
                    <div className="bg-emerald-50 rounded-lg p-5 border border-emerald-200">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="font-bold text-emerald-900 text-sm uppercase tracking-wide">Project Goals</h3>
                      </div>
                  ...
                  {hasContent(summaryData.overview.audience) && (
                    <div className="bg-emerald-50 rounded-lg p-5 border border-emerald-200">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="font-bold text-emerald-900 text-sm uppercase tracking-wide">Target Audience</h3>
                      </div>
                      <p className="text-emerald-800 leading-relaxed whitespace-pre-wrap">
                ...
                {hasContent(summaryData.overview.deliverables) && (
                  <div className="bg-emerald-50 rounded-lg p-5 border border-emerald-200">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="font-bold text-emerald-900 text-sm uppercase tracking-wide">Deliverables</h3>
                    </div>
                    <p className="text-emerald-800 leading-relaxed whitespace-pre-wrap">
                ...
                {hasContent(summaryData.overview.successMetrics) && (
                  <div className="bg-emerald-50 rounded-lg p-5 border border-emerald-200">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="font-bold text-emerald-900 text-sm uppercase tracking-wide">Success Metrics</h3>
                    </div>
                    <p className="text-emerald-800 leading-relaxed whitespace-pre-wrap">
            ...
            {!sections[1].hasData ? (
              ...
              <>
                {hasContent(summaryData.moodBoard.inspirationImages) && (
                  <div>
                    <p className="text-xs font-bold text-purple-700 uppercase tracking-wide mb-4">Design Inspiration</p>
                ...
                {hasContent(summaryData.moodBoard.websiteReferences) && (
                  <div>
                    <p className="text-xs font-bold text-purple-700 uppercase tracking-wide mb-4">Website References</p>
        ...
        {expandedSections.styleguide && (
          <CardContent className="pt-6 space-y-8">
            {!sections[2].hasData ? (
              ...
              <>
                {(hasContent(summaryData.styleGuide.colors) || hasContent(summaryData.styleGuide.customColors)) && (
                  <div>
                        ...
                        >
                          <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-2">
                              ...
                              {typo.description && <span className="text-xs text-gray-500">{typo.description}</span>}
                            </div>
                            <Button
                              variant="ghost"
                              size="sm"
                ...
                {summaryData.styleGuide.buttonStyles && Object.keys(summaryData.styleGuide.buttonStyles).length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {Object.entries(summaryData.styleGuide.buttonStyles).map(([type, btn]: [string, any]) => (
                        <div
                          key={type}
          ...
          <CardContent className="pt-6">
            {!sections[4].hasData ? (
              ...
              <div className="space-y-6">
                      ...
                      <div className="space-y-2">
                        {summaryData.technical.currentHosting && (
                          <div>
                            ...
                            <span className="text-sm text-gray-900">{summaryData.technical.currentHosting}</span>
                          </div>
                        )}
                        {summaryData.technical.proposedHosting && (
                          <div>
                            <span className="text-xs font-medium text-blue-700">Proposed: </span>
                            <span className="text-sm text-gray-900">{summaryData.technical.proposedHosting}</span>
                          </div>
                        )}
                        {summaryData.technical.hostingNotes && (
                          <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {/* Integrations & APIs */}
                  {(summaryData.technical?.thirdPartyIntegrations || summaryData.technical?.apiIntegrations) && (
                    <div className="border border-blue-200 bg-blue-50 p-4 rounded-lg">
                      <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="bg-rose-50 p-4 rounded-lg border border-rose-200">
                      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ...
                        {summaryData.content.focusKeyword && (
                          <div>
                            ...
                            <p className="text-gray-800 text-sm font-medium">{summaryData.content.focusKeyword}</p>
                          </div>
                        )}

                        {summaryData.content.metaDescription && (
                          <div className="md:col-span-2">
                            <p className="text-xs font-semibold text-rose-700 uppercase mb-1">Meta Description</p>
                        ...
                        {summaryData.content.seoKeywords && summaryData.content.seoKeywords.length > 0 && (
                          <div className="md:col-span-2 lg:col-span-4">
                            <p className="text-xs font-semibold text-rose-700 uppercase mb-2">Target Keywords</p>
                      ...
                      {summaryData.content?.competitorAnalysis && (
                        <div className="pt-3 border-t border-rose-200">
                          <p className="text-xs font-semibold text-rose-700 uppercase mb-2">Competitor Analysis</p>
                        ...
                        {summaryData.content.messagingPillars.map((pillar: any, idx: number) => {
                          ...
                          return (
                            <div key={idx} className="bg-rose-50 p-3 rounded-lg border border-rose-200">
                              ...
                              {pillarDescription && <p className="text-gray-600 text-xs">{pillarDescription}</p>}
                            </div>
                          )
                        })}
                      </div>
                      ...
                      <div className="space-y-2">
                        {summaryData.content.contentItems.map((item: any, idx: number) => {
                          ...
                          const typeStyles: Record<string, { bg: string; label: string; text: string }> = {
                            ...
                            subheading: { bg: "bg-rose-50", label: "Subheading", text: "text-sm font-semibold" },
                            cta: { bg: "bg-rose-500 text-white", label: "CTA", text: "font-semibold text-sm" },
                            body: { bg: "bg-rose-50", label: "Body", text: "text-xs" },
                            tagline: { bg: "bg-rose-100", label: "Tagline", text: "text-sm font-medium italic" },
                          }
